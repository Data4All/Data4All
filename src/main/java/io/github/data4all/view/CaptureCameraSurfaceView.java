...
import io.github.data4all.util.SystemProperties;
...
public class CaptureCameraSurfaceView extends ViewGroup implements
	...
	public void surfaceCreated(SurfaceHolder holder) {
		...
		try {
			if (mCamera != null) {
				...
				float vva = params.getVerticalViewAngle();
				float hva = params.getHorizontalViewAngle();
	...
	public void surfaceChanged(SurfaceHolder holder, int format, int width,
		...
		editor.putInt(SystemProperties.CAMERA_PREVIEW_WIDTH, mPreviewSize.width);
		editor.putInt(SystemProperties.CAMERA_PREVIEW_HEIGHT,
				mPreviewSize.height);

		if (mPhotoSize != null) {

			editor.putInt(SystemProperties.PHOTO_WIDTH, mPhotoSize.width);
			editor.putInt(SystemProperties.PHOTO_HEIGHT, mPhotoSize.height);

			editor.putInt(SystemProperties.PHOTO_ICON_WIDTH,
					mPhotoSize.width / 8);
			editor.putInt(SystemProperties.PHOTO_ICON_HEIGHT,
					mPhotoSize.height / 8);
			...
			Log.d(TAG,
					"#####VVA="
							+ settings
									.getFloat(
											SystemProperties.CAMERA_VERTICAL_VIEW_ANGLE,
											0.0f));
			...
			editor.putFloat(SystemProperties.CAMERA_DISTANCE_TO_PIC,
