...
public class ResultViewActivity extends AbstractActivity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent().hasExtra(ShowPictureActivity.CURRENT_ORIENTATION_EXTRA)) {
            TagSuggestionHandler tsh = new TagSuggestionHandler();
            tsh.setCurrent((Location) getIntent()
                    .getParcelableExtra(
                            ShowPictureActivity.CURRENT_ORIENTATION_EXTRA));
        }
         ...
         * classifiedValue).isEmpty()) { final LayoutInflater inflater =
         * getLayoutInflater(); ((LayoutInflater) this
         * .getSystemService(LAYOUT_INFLATER_SERVICE))
         * .inflate(R.drawable.footer_listviewresult, null, false);
         * listView.addFooterView(viewFooter); final TextView tVFooter =
         * ((TextView) viewFooter.findViewById(R.id.titleFooter));
         * tVFooter.setOnClickListener(this); viewFooter.setOnClickListener(new
         * OnClickListener() {
         * 
         * @Override public void onClick(View v) { createDialogAddTags();
         * 
         *           } }); }
         */

        final Button addressSuggestions = (Button) this
                .findViewById(R.id.buttonAddressSuggestions);
        addressSuggestionView = new AddressSuggestionView(this, this,
                addressSuggestions);
        ...
        listView.setOnItemClickListener(new OnItemClickListener() {
            public void onItemClick(AdapterView<?> parent, View view,
                ...
                } else if (keyList.get(position).equals(
                        res.getString(R.string.SelectTag))) {
                    addClassifiedTag();
                } else {
                ...
                new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int id) {
                        ...
                        for (Entry<Tag, String> entry : map.entrySet()) {
                            ...
                            if (tag1.getKey().equals(
                                    mapTag.get(selectedString).getKey())) {
                                Log.i(TAG, "true");
    ...
    private void output() {
        ...
        for (Entry<Tag, String> entry : element.getTags().entrySet()) {
            if (entry.getKey() != null) {
                ...
                if (Tagging.isClassifiedTag(
                        getString(tagKey.getNameRessource()), res)) {
                    try {
        ...
        if (keyList.size() >= 1) {
            ...
            classifiedMap = Tagging.classifiedValueMap(
                    tagMap.get(keyList.get(0)).getClassifiedValues(), this,
                    false);
            ...
            String value = null;
            try {
                value = tagKey.getNamedValue(this, element.getTags()
                        .get(tagKey).replaceAll(":", "_"));
            } catch (IllegalArgumentException e) {
            ...
            if (classi != null) {
                keyList = Tagging.getUnclassifiedTags(classi, this, keyList,
                        element);
                endList = Tagging.addUnclassifiedValue(element, endList,
                        keyList, this);
            }
    ...
    private void changeClassifiedTag(final String selectedString) {
        ...
        final CharSequence[] showArray;
        final ArrayList<String> arrayList = (ArrayList<String>) Tagging
                .ClassifiedValueList(tagMap.get(selectedString)
        ...
        dialog.show();
        /**
         * Log.i(TAG, "Classified Tag"); final AlertDialog.Builder alertDialog =
         * new AlertDialog.Builder( ResultViewActivity.this,
         * android.R.style.Theme_Holo_Dialog_MinWidth);
         * alertDialog.setTitle(R.string.SelectTag); final CharSequence[]
         * showArray; showArray =
         * Tagging.ClassifiedValueList(tagMap.get(selectedString)
         * .getClassifiedValues(), res); classifiedMap =
         ...
         *           final String value = (String) showArray[which]; final
         *           String realValue = classifiedMap.get(value).getValue();
         *           Log.i(TAG, "Value " + realValue); Log.i(TAG,
         *           tagMap.get(selectedString).toString()); Tag tagKey = null;
         *           for (Entry<Tag,String> entry :
         *           element.getTags().entrySet()) { tagKey = entry.getKey();
         *           break; } element.removeTag(tagKey);
         *           map.putAll(element.getTags()); map.remove(tagKey);
         *           Log.i(TAG, "MAP" + map.toString()); element.clearTags();
         * 
         * 
         *           element.addOrUpdateTag(tagMap.get(selectedString),
         *           realValue);
         *           Tagging.compareUnclassifiedTags(classifiedMap.get(value),
         *           map); element.addTags(map);
         *           ResultViewActivity.this.output(); } });
         * 
         *           alertDialog.setCancelable(false); alert =
         *           alertDialog.create();
         ...
         *           alert.dismiss();
         *           element.removeTag(tagMap.get(selectedString));
         *           addClassifiedTag(); return true; } return true; } });
         *           alert.show();
         */
    }
    ...
    private void changeUnclassifiedTag(final String selectedString) {
        ...
        next.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if (text.getText().toString().matches("")) {
                    dialog.dismiss();
                    ResultViewActivity.this.changeUnclassifiedTag(keyList
                            .get(keyList.indexOf(selectedString) + 1));
                } else {
                    for (Entry<Tag, String> entry : map.entrySet()) {
                        Tag tag1 = entry.getKey();
                        if (tag1.getKey().equals(tag.getKey())) {
    ...
    public void onClick(View v) {
        ...
        case R.id.buttonResultToCamera:
            ...
            break;
        /**
         * case R.id.titleFooter: createDialogAddTags(); break;
         */
    ...
    private void askForGalleryDelete() {
        if (getIntent().hasExtra(Gallery.GALLERY_ID_EXTRA)) {
            final long id = getIntent().getLongExtra(Gallery.GALLERY_ID_EXTRA,
                    0);
     ...
     */
    /**
     * private void createDialogAddTags() { final AlertDialog.Builder
     * alertDialog = new AlertDialog.Builder( ResultViewActivity.this,
     * android.R.style.Theme_Holo_Dialog_MinWidth); Log.i(TAG, "bla" +
     * classifiedValue); final List<Tag> list =
     * Tagging.getAllNonSelectedTags(element.getTags(), classifiedValue);
     * Log.i(TAG, list.toString()); final String[] listString; listString =
     * Tagging.TagsToStringRes(list, res); alertDialog.setItems(listString, new
     * DialogInterface.OnClickListener() {
     * 
     * @Override public void onClick(DialogInterface dialog, int which) {
     *           Log.i(TAG, listString[which]);
     *           changeUnclassifiedTag(listString[which]); alert.dismiss(); }
     *           }); list.clear(); alert = alertDialog.create();
     *           alert.getWindow().setBackgroundDrawable( new
     *           ColorDrawable(android.graphics.Color.TRANSPARENT));
     *           alert.show();
     * 
