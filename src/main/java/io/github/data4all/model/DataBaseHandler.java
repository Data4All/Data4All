...
public class DataBaseHandler extends SQLiteOpenHelper {
	...
	public void onCreate(SQLiteDatabase db) {
		...
		String CREATE_WAYS_TABLE = "CREATE TABLE " + TABLE_WAY + " ("
				...
				+ KEY_NODEID + "))";
		String CREATE_RELATIONMEMBERS_TABLE = "CREATE TABLE "
				+ TABLE_RELATIONMEMBER + " (" + KEY_REF
				+ " INTEGER PRIMARY KEY," + KEY_TYPE + " TEXT," + KEY_ROLE
				+ " TEXT" + ")";
		String CREATE_RELATIONS_TABLE = "CREATE TABLE " + TABLE_RELATION + " ("
				+ KEY_OSMID + " INTEGER NOT NULL," + KEY_RELATIONMEMBER
				+ " INTEGER NOT NULL," + " PRIMARY KEY (" + KEY_OSMID + ", "
				+ KEY_RELATIONMEMBER + "))";
		String CREATE_OSMTAGMAPS_TABLE = "CREATE TABLE " + TABLE_OSMTAGMAP
				...
				+ KEY_OSMID + ", " + KEY_KEY + "))";
		String CREATE_OSMPARENTLISTS_TABLE = "CREATE TABLE "
				+ TABLE_OSMPARENTLIST + " (" + KEY_OSMID + " INTEGER NOT NULL,"
				+ KEY_RELATIONID + " INTEGER NOT NULL," + " PRIMARY KEY ("
				+ KEY_OSMID + ", " + KEY_RELATIONID + "))";
		...
		String CREATE_BOUNDINGBOXS_TABLE = "CREATE TABLE " + TABLE_BOUNDINGBOX
				+ " (" + KEY_MINLAT + " REAL NOT NULL," + KEY_MINLON
				+ " REAL NOT NULL," + KEY_MAXLAT + " REAL NOT NULL,"
				+ KEY_MAXLON + " REAL NOT NULL," + " PRIMARY KEY ("
				+ KEY_MINLAT + ", " + KEY_MINLON + ", " + KEY_MAXLAT + ", "
				+ KEY_MAXLON + "))";
	...
	// db.close();
	// }
	// public BoundingBox getBoundingBox (int id){ //evtl. für BoundingBox noch
	// ne id hinzufügen
	// SQLiteDatabase db = getReadableDatabase();
	//
	// //TODO: use ID??
	// Cursor cursor = db.query(TABLE_BOUNDINGBOX, new String[]{KEY_MINLAT,
	// KEY_MINLON, KEY_MAXLAT, KEY_MAXLON},
	...
	// BoundingBox(Double.parseDouble(cursor.getString(0)),
	...
	//
	//
	// // db.delete(TABLE_BOUNDINGBOX, KEY_ID + "=?", new
	// String[]{String.valueOf(boundingBox.getID())});
	//
	// db.close();
	// }
	//
	...
	//
	// // db.close();
	// // return db.update(TABLE_BOUNDINGBOX, values, KEY_ID + "=?", new
	// String[]{String.valueOf(boundingBox.getID())});
	// return 0;
	// }
	//
	...
	public User getUser(String username) {
		...
		Cursor cursor = db.query(TABLE_USER, new String[] { KEY_USERNAME,
				KEY_TOKEN, KEY_LOGINSTATUS }, KEY_USERNAME + "=?",
	...
	public Node getNode(long id) {
		...
		Node node = new Node(Long.parseLong(cursor.getString(0)),
				getOsmElementOsmVersion(id), Double.parseDouble(cursor
	...
	public int updateNode(Node node) {
		...
		count += updateParentRelation(node.getOsmId(),
				node.getParentRelations());
	...
	/**
	 * This method creates and stores a new relation in the database. The data
	 * is taken from the {@link Relation} object that is passed to the method.
	 * 
	 * @param relation
	 *            the {@link Relation} object from which the data will be taken
	 */
	public void createRelation(Relation relation) {
		...
		if (relation.getParentRelations() != null) {
			createParentRelation(relation.getOsmId(),
					relation.getParentRelations());
		}
	...
	/**
	 * This method returns the data for a specific relation stored in the
	 * database and creates the corresponding {@link Relation} object.
	 * 
	 * @param id
	 *            the id of the desired relation
	 * @return a {@link Relation} object for the desired relation
	 */
	public Relation getRelation(long id) {
		SQLiteDatabase db = getReadableDatabase();

		Cursor cursor = db.query(TABLE_RELATION, new String[] { KEY_OSMID,
				KEY_RELATIONMEMBER }, KEY_OSMID + "=?",
				new String[] { String.valueOf(id) }, null, null, null, null);
	 ...
	 * 
	 * @param relation
	 *            the {@link Relation} object whose data should be deleted
	 */
	public void deleteRelation(Relation relation) {
		SQLiteDatabase db = getWritableDatabase();

		db.delete(TABLE_RELATION, KEY_OSMID + "=?",
				new String[] { String.valueOf(relation.getOsmId()) });
	...
	/**
	 * This method returns the number of relations currently stored in the
	 * database.
	 * 
	...
	/**
	 * This method updates the data for a specific relation stored in the
	 * database.
	 * 
	 * @param relation
	 *            the {@link Relation} object for which the data should be
	 *            updated
	 * @return the number of rows that have been updated
	 */
	public int updateRelation(Relation relation) {
		...
		for (RelationMember rm : relation.getMembers()) {
			values.put(KEY_RELATIONMEMBER, rm.getRef());
			count += db.update(TABLE_RELATION, values, KEY_OSMID + "=?",
					new String[] { String.valueOf(relation.getOsmId()) });
		}
		...
		count += updateParentRelation(relation.getOsmId(),
				relation.getParentRelations());
	...
	/**
	 * This method returns a list of all relations stored in the database and
	 * creates corresponding {@link Relation} objects.
	 * 
	...
	public List<Relation> getAllRelation() {
		...
		if (cursor.moveToFirst()) {
			do {
				...
				Relation relation = new Relation(Long.parseLong(cursor
						.getString(0)),
						getOsmElementOsmVersion(Long.parseLong(cursor
								.getString(0))));
				relation.addMembers(theseMembers, false);
				relation.setTags(getTagSortedMap(relation.getOsmId()));
				relation.addParentRelations(getParentRelationList(relation
						.getOsmId()));
	...
	/**
	 * This method creates and stores a new relation member in the database. The
	 * data is taken from the {@link RelationMember} object that is passed to
	 * the method.
	 * 
	 * @param relationMember
	 *            the {@link RelationMember} object from which the data will be
	 *            taken
	 */
	...
	/**
	 * This method returns the data for a specific relation member stored in the
	 * database and creates the corresponding {@link RelationMember} object.
	 * 
	 * @param ref
	 *            the reference to the desired relation member
	 * @return a {@link RelationMember} object for the desired relation member
	 */
	public RelationMember getRelationMember(long ref) {
		SQLiteDatabase db = getReadableDatabase();

		Cursor cursor = db.query(TABLE_RELATIONMEMBER, new String[] { KEY_REF,
				KEY_TYPE, KEY_ROLE }, KEY_REF + "=?",
	 ...
	 * 
	 * @param relationMember
	 *            the {@link RelationMember} object whose data should be deleted
	 */
	public void deleteRelationMember(RelationMember relationMember) {
		SQLiteDatabase db = getWritableDatabase();

		db.delete(TABLE_RELATIONMEMBER, KEY_REF + "=?",
	...
	/**
	 * This method deletes a specific relation member from the database by the
	 * refId.
	 * 
	 * @param refId
	 *            the ref of an RelationMember {@link RelationMember} object
	 *            whose data should be deleted
	 */
	...
	/**
	 * This method returns the number of relation members currently stored in
	 * the database.
	 * 
	...
	public int getRelationMemberCount() {
		...
		Cursor cursor = db.rawQuery("SELECT * FROM " + TABLE_RELATIONMEMBER,
				null);
		int count = cursor.getCount();
		cursor.close();
		db.close();
	...
	/**
	 * This method updates the data for a specific relation member stored in the
	 * database.
	 * 
	 * @param relationMember
	 *            the {@link RelationMember} object for which the data should be
	 *            updated
	 * @return the number of rows that have been updated
	 */
	...
	/**
	 * This method returns a list of all relation members stored in the database
	 * and creates corresponding {@link RelationMember} objects.
	 * 
	 ...
	 * @param osmVersion
	 *            the version of the OsmElement object
	 * 
	 */
	...
	/**
	 * This method creates and stores the ParentRelationList of an OsmElement in
	 * the database.
	 * 
	 * @param osmId
	 *            the id of the OsmElement object
	 * @param parentRelation
	 *            the List of Parent Relations of the OsmElement object
	 * 
	 */
	...
	private int updateOsmElement(long osmId, long osmVersion) {
		...
		int count = db.update(TABLE_OSMELEMENT, values, KEY_OSMID + "=?",
	...
	/**
	 * This method updates and stores the ParentRelationList of an OsmElement in
	 * the database.
	 * 
	 * @param osmId
	 *            the id of the OsmElement object
	...
	private int updateParentRelation(long osmId, List<Relation> parentRelations) {
		...
		for (Relation r : parentRelations) {
			values.put(KEY_RELATIONID, r.getOsmId());
			count += db.update(TABLE_OSMPARENTLIST, values, KEY_OSMID + "=?",
					new String[] { String.valueOf(osmId) });
		}
		db.close();
		return count;
	 ...
	 * 
	 * @param osmId
	 *            the id of an OsmElement object whose data should be deleted
	 */
	private void deleteOsmElement(long osmId) {
		SQLiteDatabase db = getWritableDatabase();

		db.delete(TABLE_OSMELEMENT, KEY_OSMID + "=?",
				new String[] { String.valueOf(osmId) });
	...
	/**
	 * This method deletes a specific ParentRelationList of an OsmElement from
	 * the database.
	 * 
	 * @param osmId
	 *            the id of the OsmElement object whose List data should be
	 *            deleted
	 */
	private void deleteParentRelationList(long osmId) {
		SQLiteDatabase db = getWritableDatabase();

		db.delete(TABLE_OSMPARENTLIST, KEY_OSMID + "=?",
				new String[] { String.valueOf(osmId) });
	...
	/**
	 * This method returns the data for a specific way stored in the database
	 * and creates the corresponding List object.
	 * 
	 * @param osmId
	...
	private List<Relation> getParentRelationList(long osmId) {
		...
		SQLiteDatabase db = getReadableDatabase();
		Cursor cursor = db.query(TABLE_OSMPARENTLIST,
				new String[] { KEY_OSMID, KEY_RELATIONID }, KEY_OSMID + "=?",
	...
	/**
	 * This method returns the data for a specific way stored in the database
	 * and returns the version of an OsmElement.
	 * 
	 * @param osmId
	 *            the id of the OsmElement
	...
	private long getOsmElementOsmVersion(long osmId) {
		...
		Cursor cursor = db.query(TABLE_OSMELEMENT, new String[] { KEY_OSMID,
				KEY_OSMVERSION }, KEY_OSMID + "=?",
				new String[] { String.valueOf(osmId) }, null, null, null, null);
